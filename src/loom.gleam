...
/// Performs a fuzzy search on the trie, returning all keys and values within the 
/// specified Levenshtein distance (deviation) from the pattern.
///
/// This function explores multiple branches to account for insertions, deletions, 
/// and substitutions.
///
/// ## Examples
///
...
/// |> insert(["c", "a", "t"], Nil)
/// |> insert(["c", "a", "r"], Nil)
/// |> fuzzy_search(["c", "a", "s", "e"], 2)
/// // -> [#(["c", "a", "t"], Nil), #(["c", "a", "r"], Nil)]
/// ```
///
/// ```gleam
/// new()
/// |> insert(string.to_graphemes("rust"), Nil)
/// |> fuzzy_search(string.to_graphemes("dust"), 1)
/// // -> [#(["r", "u", "s", "t"], Nil)]
/// ```
/// 
pub fn fuzzy_search(
  ...
  fuzzy_search_loop(trie, [], [], pattern, deviation)
  |> list.unique
}

fn fuzzy_search_loop(
  trie: Trie(k, v),
  key_acc: List(k),
  found_acc: List(#(List(k), v)),
  pattern: List(k),
  deviation: Int,
) -> List(#(List(k), v)) {
  use <- bool.guard(deviation < 0, found_acc)

  let found_acc = case pattern, trie.value {
    [], Some(val) -> [#(list.reverse(key_acc), val), ..found_acc]
    _, _ -> found_acc
  }

  // key skips
  let found_acc = case pattern {
    [_, ..rest] ->
      fuzzy_search_loop(trie, key_acc, found_acc, rest, deviation - 1)
    [] -> found_acc
  }
