...
pub fn fuzzy_search_test() {
  let trie =
    ...
    |> loom.insert(string.to_graphemes("bar"), 3)
    |> loom.insert(string.to_graphemes("boat"), 4)
    |> loom.insert(string.to_graphemes("boot"), 5)
    |> loom.insert(string.to_graphemes("b"), 6)
    |> loom.insert(string.to_graphemes("ba"), 7)
    |> loom.insert(string.to_graphemes("batty"), 8)
    |> loom.insert(string.to_graphemes("baaat"), 9)
    |> loom.insert(string.to_graphemes("bt"), 10)
    |> loom.insert(string.to_graphemes("t"), 11)

  assert trie
    |> loom.fuzzy_search(string.to_graphemes("bat"), 1)
    |> list.sort(fn(a, b) {
      string.compare(string.concat(a.0), string.concat(b.0))
    })
    == [
      #(string.to_graphemes("ba"), 7),
      #(string.to_graphemes("bar"), 3),
      #(string.to_graphemes("bat"), 1),
      #(string.to_graphemes("boat"), 4),
      #(string.to_graphemes("bt"), 10),
      #(string.to_graphemes("cat"), 2),
    ]
}
